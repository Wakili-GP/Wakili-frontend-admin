...
const UserManagement = () => {
  ...
  const handleSearch = async (query: string) => {
    ...
    if (query.trim()) {
      try {
        const results = await userManagementApi.searchUsers(query);
  ...
  const handleSuspend = async (id: string) => {
    try {
      ...
      const result = await userManagementApi.suspendUser({
        userId: id,
      });
      if (result) {
  ...
  const handleReinstate = async (id: string) => {
    try {
      ...
      const result = await userManagementApi.reinstateUser({
        userId: id,
      });
      if (result) {
        setUsers(users.map((u) => (u.id === id ? result : u)));
        ...
        toast.error("فشل في استعادة الحساب");
      }
    } catch (err) {
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-12">
          <div className="text-center">
            ...
            <p className="text-slate-400">جاري تحميل المستخدمين...</p>
          </div>
        </div>
      ) : (
        <>
                            ...
                            >
                              ...
                              {user.status === "active" ? (
                                <DropdownMenuItem
                                  ...
                                  className="cursor-pointer text-red-400 focus:text-red-300 focus:bg-red-500/10 disabled:opacity-50"
                                >
                                ...
                                <DropdownMenuItem
                                  ...
                                  disabled={isReinstating || isSuspending}
                                  className="text-emerald-400 focus:text-emerald-300 focus:bg-emerald-500/10 disabled:opacity-50"
                                >
                                  {isReinstating ? (
                                  ...
                                  {isReinstating ? "جاري..." : "استعادة الحساب"}
                                </DropdownMenuItem>
                              )}
                            </DropdownMenuContent>
                          </DropdownMenu>
                  ...
                  <DialogFooter>
                    {selectedUser.status === "active" ? (
                      <Button
                        ...
                        className="cursor-pointer border-red-500/20 text-red-400 hover:bg-red-500/10 disabled:opacity-50"
                      >
                        {isSuspending ? (
                          <Loader className="w-4 h-4 ml-2 animate-spin" />
                        ) : (
                        ...
                        {isSuspending ? "جاري..." : "تعليق الحساب"}
                      </Button>
                    ) : (
                      <Button
                        onClick={() => handleReinstate(selectedUser.id)}
                        disabled={isReinstating}
                        className="cursor-pointer bg-emerald-500 hover:bg-emerald-600 text-white disabled:opacity-50"
                      >
                        {isReinstating ? (
                          <Loader className="w-4 h-4 ml-2 animate-spin" />
                        ) : (
            ...
            <DialogContent className="bg-slate-800 border-slate-700">
              <DialogHeader>
                ...
                <DialogDescription className="text-slate-400">
                  هل أنت متأكد من تعليق حساب {selectedUser?.name}؟ سيتم منع
